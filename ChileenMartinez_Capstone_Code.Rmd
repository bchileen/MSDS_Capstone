...
```{r}
build_lstm_model_simple <- function(sequence_length, n_features, forecast_horizon) {
...
train_lstm <- function(lstm_data, dataset_name, threshold,
                              epochs = 200, batch_size = 8, patience = 20) {
  ...
  # Binary classification (1-step ahead)
  ...
  f1 <- 2 * sum(actual_class == "YES" & pred_class == "YES") /
    (2 * sum(actual_class == "YES" & pred_class == "YES") +
       sum(actual_class == "NO" & pred_class == "YES") +
       sum(actual_class == "YES" & pred_class == "NO"))
  ...
  # Create metrics data frame (ADDED)
  metrics <- data.frame(
    Dataset = dataset_name,
    Model = "LSTM",
  ...
  list(
    ...
    horizon_metrics = horizon_metrics,  # ADDED
    predictions = data.frame(  # ADDED for consistency with xGBoost
...
for (i in 1:nrow(viable_pools)) {
  ...
  if (!is.null(pool_lstm_metrics) && nrow(pool_lstm_metrics) > 0) {
    cat(sprintf("\nCompiled %d pool LSTM results\n", nrow(pool_lstm_metrics)))
    print(pool_lstm_metrics |> arrange(Test_RMSE_1Step))
  }
} else {
  pool_lstm_metrics <- NULL
  cat("\nNo pool LSTM results to compile\n")
}
    
    # Plot for each pool (optional)
...
# Compile River Model Results 
```{r compile-results}
cat("\n")
cat("╔════════════════════════════════════════════════════════════════════════╗\n")
cat("║                    COMPILING MODEL RESULTS                             ║\n")
cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")

# ----------------------------------------------------------------------------
# 1. RIVER-LEVEL REGRESSION RESULTS
# ----------------------------------------------------------------------------
...
if (!is.null(IWW_lstm_result)) {
  iww_lstm_reg <- data.frame(
    Dataset = "Illinois Waterway",
    Model = "LSTM",
    Test_RMSE = sqrt(mean((IWW_lstm_result$test_pred[,1] - IWW_lstm_result$test_actual[,1])^2)),
    Test_MAE = mean(abs(IWW_lstm_result$test_pred[,1] - IWW_lstm_result$test_actual[,1])),
    River = "Illinois Waterway",
    Type = "ML"
  )
  river_regression <- bind_rows(river_regression, iww_lstm_reg)
}

if (!is.null(Miss_lstm_result)) {
  miss_lstm_reg <- data.frame(
    Dataset = "Mississippi River",
    Model = "LSTM",
    Test_RMSE = sqrt(mean((Miss_lstm_result$test_pred[,1] - Miss_lstm_result$test_actual[,1])^2)),
    Test_MAE = mean(abs(Miss_lstm_result$test_pred[,1] - Miss_lstm_result$test_actual[,1])),
    River = "Mississippi River",
    Type = "ML"
  )
  river_regression <- bind_rows(river_regression, miss_lstm_reg)
}
...
# Add LSTM classification
if (!is.null(IWW_lstm_result)) {
  iww_lstm_class <- data.frame(
    Dataset = "Illinois Waterway",
  ...
  classification_results <- bind_rows(classification_results, iww_lstm_class)
}

if (!is.null(Miss_lstm_result)) {
  miss_lstm_class <- data.frame(
    Dataset = "Mississippi River",
  ...
  classification_results <- bind_rows(classification_results, miss_lstm_class)
}
...
```
##Compile Pool Level Model results
```{r}
if (!is.null(pool_xgb_metrics) && !is.null(pool_lstm_metrics)) {
  
  cat("\n")
  cat("╔════════════════════════════════════════════════════════════════════════╗\n")
  cat("║              POOL-LEVEL MODEL SELECTION                                ║\n")
  cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")
  
  # Standardize column names for comparison
  pool_xgb_compare <- pool_xgb_metrics |>
    select(Pool, River, Model, Test_RMSE, Test_MAE, Test_R2, 
           Accuracy, F1_Score, N_Samples) |>
    mutate(Model = "xGBoost")
  
  pool_lstm_compare <- pool_lstm_metrics |>
    select(Dataset, Model, Test_RMSE_1Step, Test_MAE_1Step, 
           Correlation, Accuracy, F1_Score) |>
    separate(Dataset, into = c("Pool_Label", "Pool"), sep = " ") |>
    left_join(pool_info |> select(pool, river, n_samples), 
              by = c("Pool" = "pool")) |>
    select(Pool, River = river, Model, 
           ...
           N_Samples = n_samples) |>
    mutate(Model = "LSTM",
           Test_R2 = Test_R2^2)  # Convert correlation to R²
  
  # Combine results
  pool_comparison <- bind_rows(pool_xgb_compare, pool_lstm_compare)
  
  # Select best model per pool based on RMSE
  best_pool_models <- pool_comparison |>
    group_by(Pool, River) |>
    slice_min(Test_RMSE, n = 1) |>
    ungroup() |>
    arrange(River, Pool)
  
  cat("\n=== Best Model Per Pool (by RMSE) ===\n")
  print(best_pool_models |> 
          select(Pool, River, Model, Test_RMSE, Test_MAE, F1_Score))
  
  # Summary statistics
  model_summary <- best_pool_models |>
    group_by(Model) |>
    summarise(
      N_Pools = n(),
  ...
  # River-specific breakdown
  river_model_summary <- best_pool_models |>
    group_by(River, Model) |>
    summarise(N_Pools = n(), .groups = "drop") |>
    pivot_wider(names_from = Model, values_from = N_Pools, values_fill = 0)
  
  cat("\n=== Model Selection by River ===\n")
  print(river_model_summary)
  
  # Save results
  write_csv(pool_comparison, "./Output/Pool_Models/Pool_Model_Comparison.csv")
  write_csv(best_pool_models, "./Output/Pool_Models/Best_Pool_Models.csv")
  
  # ============================================================================
  # VISUALIZATION: Model Selection Map
  # ============================================================================
  
  pool_selection_map <- best_pool_models |>
    left_join(pool_info |> select(pool, river, center_rm), 
              by = c("Pool" = "pool", "River" = "river")) |>
    mutate(x_pos = ifelse(River == "IL", 2, 1))
  
  p_model_selection <- ggplot(pool_selection_map, aes(x = x_pos, y = center_rm)) +
    geom_line(aes(group = River), color = "lightblue", linewidth = 10, alpha = 0.3) +
    geom_point(aes(fill = Model, size = N_Samples), 
               shape = 21, color = "black", alpha = 0.8) +
    geom_text(aes(label = Pool), hjust = -0.3, size = 3) +
    scale_fill_manual(
      values = c("xGBoost" = "#1b9e77", "LSTM" = "#d95f02"),
      name = "Best Model"
    ) +
    scale_size_continuous(range = c(4, 12), name = "N Samples") +
    scale_x_continuous(
      breaks = c(1, 2),
    ...
    labs(
      title = "Best Model Selection by Pool",
      subtitle = "Based on lowest test RMSE",
      y = "River Mile",
      x = ""
    ) +
    theme_minimal() +
  ...
  print(p_model_selection)
  ggsave("./Output/Pool_Models/Pool_Model_Selection_Map.png", 
         width = 12, height = 14)
  ...
  p_pool_comparison <- ggplot(pool_comparison, 
                               aes(x = reorder(Pool, Test_RMSE), 
                                   y = Test_RMSE, fill = Model)) +
    geom_col(position = "dodge", alpha = 0.8) +
    facet_wrap(~River, scales = "free_x") +
    scale_fill_manual(values = c("xGBoost" = "#1b9e77", "LSTM" = "#d95f02")) +
    labs(
      title = "Pool-Level Model Performance Comparison",
      x = "Pool", y = "Test RMSE (ft/yr)"
    ) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  print(p_pool_comparison)
  ggsave("./Output/Pool_Models/Pool_Performance_Comparison.png", 
         width = 14, height = 8)
  
  cat("\n✓ Pool-level model selection complete\n")
  
} else {
  cat("\nWARNING: Cannot compare pool models - missing xGBoost or LSTM results\n")
}
```
...
# LSTM plots - CORRECTED
if (!is.null(IWW_lstm_result) && !is.null(Miss_lstm_result)) {
...
generate_pool_recommendations <- function(best_pool_models, 
                                          pool_comparison = NULL,
                                          pool_info) {
  
  cat("\n")
  cat("╔════════════════════════════════════════════════════════════════════════╗\n")
  cat("║              POOL-LEVEL MODEL RECOMMENDATIONS                          ║\n")
  cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")
  ...
  overall_stats <- best_pool_models |>
    summarise(
      N_Pools = n(),
      Avg_RMSE = mean(Test_RMSE, na.rm = TRUE),
      Avg_F1 = mean(F1_Score, na.rm = TRUE),
      N_xGBoost = sum(Model == "xGBoost"),
      N_LSTM = sum(Model == "LSTM")
    )
  
  cat("Overall Pool-Level Performance:\n")
  cat(sprintf("  • Pools Modeled: %d\n", overall_stats$N_Pools))
  cat(sprintf("  • Average RMSE: %.4f ft/yr\n", overall_stats$Avg_RMSE))
  cat(sprintf("  • Average F1 Score: %.3f\n", overall_stats$Avg_F1))
  cat(sprintf("  • Model Selection: %d xGBoost, %d LSTM\n\n", 
              overall_stats$N_xGBoost, overall_stats$N_LSTM))
  
  # ----------------------------------------------------------------------------
  # River-Specific Summaries
  # ----------------------------------------------------------------------------
  
  for (river_code in c("IL", "UM")) {
    
    river_name <- ifelse(river_code == "IL", "Illinois Waterway", "Mississippi River")
    
    river_pools <- best_pool_models |>
      filter(River == river_code)
    
    if (nrow(river_pools) == 0) next
    
    cat(sprintf("=== %s ===\n", river_name))
    
    # Best model for this river
    best_model <- river_pools |>
      slice_min(Test_RMSE, n = 1)
    
    # Model preference
    n_xgb <- sum(river_pools$Model == "xGBoost")
    n_lstm <- sum(river_pools$Model == "LSTM")
    
    cat(sprintf("  Pools: %d\n", nrow(river_pools)))
    cat(sprintf("  Best Performing Pool: %s (RMSE: %.4f, F1: %.3f, Model: %s)\n",
                best_model$Pool, best_model$Test_RMSE, 
                best_model$F1_Score, best_model$Model))
    cat(sprintf("  Model Selection: %d xGBoost, %d LSTM\n", n_xgb, n_lstm))
    
    # Recommendation based on model preference
    if (n_xgb > n_lstm) {
      cat("  → Recommendation: xGBoost performs better overall\n")
      cat("     • Use for operational dredging decisions\n")
      cat("     • Faster predictions, easier to interpret\n")
    } else if (n_lstm > n_xgb) {
      cat("  → Recommendation: LSTM captures temporal patterns better\n")
      cat("     • Use for long-term planning\n")
      cat("     • Better for pools with strong seasonal patterns\n")
    } else {
      cat("  → Recommendation: Mixed performance\n")
      cat("     • Use best model per pool\n")
      cat("     • Pool-specific characteristics drive selection\n")
    }
    ...
    cat("\n  Top 3 Performing Pools:\n")
    for (i in 1:nrow(top_3)) {
      cat(sprintf("    %d. Pool %s: RMSE=%.4f, F1=%.3f (%s)\n",
                  i, top_3$Pool[i], top_3$Test_RMSE[i], 
                  top_3$F1_Score[i], top_3$Model[i]))
    }
    
    # Bottom 3 pools (needing improvement)
    bottom_3 <- river_pools |>
      arrange(desc(Test_RMSE)) |>
      head(3)
    
    cat("\n  Pools Needing Improvement:\n")
    for (i in 1:nrow(bottom_3)) {
      cat(sprintf("    %d. Pool %s: RMSE=%.4f, F1=%.3f (%s)\n",
                  i, bottom_3$Pool[i], bottom_3$Test_RMSE[i], 
                  bottom_3$F1_Score[i], bottom_3$Model[i]))
    }
  ...
  if (!is.null(pool_comparison)) {
    
    head_to_head <- pool_comparison |>
      group_by(Pool, River) |>
      filter(n() == 2) |>
      arrange(Pool, River, Test_RMSE) |>
      summarise(
        Best_Model = first(Model),
        RMSE_Diff = last(Test_RMSE) - first(Test_RMSE),
        Better_By_Pct = (last(Test_RMSE) - first(Test_RMSE)) / last(Test_RMSE) * 100,
        .groups = "drop"
      )
    ...
    if (nrow(head_to_head) > 0) {
      ...
      cat(sprintf("Pools with both models: %d\n\n", nrow(head_to_head)))
      
      xgb_wins <- sum(head_to_head$Best_Model == "xGBoost")
      lstm_wins <- sum(head_to_head$Best_Model == "LSTM")
      
      cat(sprintf("  xGBoost wins: %d (%.1f%%)\n", 
                  xgb_wins, xgb_wins/nrow(head_to_head)*100))
      cat(sprintf("  LSTM wins: %d (%.1f%%)\n", 
                  lstm_wins, lstm_wins/nrow(head_to_head)*100))
      cat(sprintf("  Average improvement: %.1f%%\n\n", 
                  mean(abs(head_to_head$Better_By_Pct))))
      
      # Biggest differences
      biggest_diffs <- head_to_head |>
        arrange(desc(abs(Better_By_Pct))) |>
        head(3)
      
      cat("  Largest Performance Differences:\n")
      for (i in 1:nrow(biggest_diffs)) {
        cat(sprintf("    %d. Pool %s: %s better by %.1f%%\n",
                    i, biggest_diffs$Pool[i], biggest_diffs$Best_Model[i],
                    abs(biggest_diffs$Better_By_Pct[i])))
      }
      cat("\n")
    }
  }
  ...
  cat("=== Deployment Recommendations ===\n\n")
  
  cat("Immediate Actions:\n")
  cat("  • Deploy best model for each pool\n")
  cat("  • Set up automated weekly predictions\n")
  cat("  • Monitor high-RMSE pools more frequently\n\n")
  
  cat("Model Selection Guidelines:\n")
  cat("  • xGBoost: Use when interpretability and speed are priorities\n")
  cat("  • LSTM: Use when temporal patterns are strong\n")
  cat("  • Consider ensemble methods for critical pools\n\n")
  
  cat("Data Collection Priorities:\n")
  
  # Identify pools with low sample counts
  low_sample_pools <- best_pool_models |>
    left_join(pool_info |> select(pool, river, n_samples),
              by = c("Pool" = "pool", "River" = "river")) |>
    filter(n_samples < median(n_samples, na.rm = TRUE)) |>
    arrange(n_samples)
  
  if (nrow(low_sample_pools) > 0) {
    cat(sprintf("  • Increase survey frequency in %d pools with limited data\n", 
                nrow(low_sample_pools)))
    cat("  • Priority pools for additional data collection:\n")
    for (i in 1:min(3, nrow(low_sample_pools))) {
      cat(sprintf("    - Pool %s: %d samples (RMSE: %.4f)\n",
                  low_sample_pools$Pool[i], 
                  low_sample_pools$n_samples[i],
                  low_sample_pools$Test_RMSE[i]))
    }
  }
...
if (exists("best_pool_models") && !is.null(best_pool_models)) {
  
  generate_pool_recommendations(
    best_pool_models = best_pool_models,
    pool_comparison = if (exists("pool_comparison")) pool_comparison else NULL,
    pool_info = pool_info
  )
  
  # Save to file
  sink("./Output/Pool_Models/Pool_Recommendations.txt")
  generate_pool_recommendations(
    best_pool_models = best_pool_models,
    pool_comparison = if (exists("pool_comparison")) pool_comparison else NULL,
    pool_info = pool_info
  ...
  cat("✓ Pool recommendations saved to ./Output/Pool_Models/Pool_Recommendations.txt\n")
  
} else {
  cat("\nWARNING: Cannot generate pool recommendations - missing best_pool_models\n")
}
