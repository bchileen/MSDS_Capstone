...
```{r}
train_lstm_simple <- function(lstm_data, dataset_name, threshold,
...
library(keras)
# Train and evaluate
IWW_lstm_result <- train_lstm_simple(
...
# Train and evaluate
Miss_lstm_result <- train_lstm_simple(
...
### Results for River Level
```{r}
lstm_river_results <- data.frame(
  River = c("Illinois Waterway"),
  Accuracy = IWW_lstm_result$accuracy,
  F1 = IWW_lstm_result$f1
)
print(lstm_river_results)
```

### Results for Pool Level
```{r}
pool_results_table <- do.call(rbind, lapply(names(pool_lstm_results), function(key) {
  res <- pool_lstm_results[[key]]
  data.frame(
    Pool = key,
