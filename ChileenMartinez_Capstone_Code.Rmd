...
#' Build LSTM model with architecture designed to leverage autoregressive features
#' 
#' Architecture notes:
#' - Multiple LSTM layers to capture complex temporal patterns
#' - Batch normalization for training stability
#' - Dropout for regularization (prevents overfitting to AR features)
#' - L2 regularization on kernels
...
build_lstm_model <- function(sequence_length,
 ...
 cat(sprintf("\nBuilding LSTM model:\n"))
 cat(sprintf("  Input shape: (%d timesteps, %d features)\n", sequence_length, n_features))
 cat(sprintf("  Output shape: %d predictions\n", forecast_horizon))
 cat(sprintf("  LSTM units: %s\n", paste(lstm_units, collapse = " → ")))
 ...
 # Third/Final LSTM layer - return_sequences = FALSE for final
 if (length(lstm_units) >= 3) {
   x <- x |>
     layer_lstm(units = lstm_units[3], 
   ...
   x <- x |>
     layer_lstm(units = lstm_units[length(lstm_units)], 
                return_sequences = FALSE)
 }
...
## Training model at river level
```{r}
#' Train LSTM with autoregressive features + comprehensive evaluation
#' 
#' This function:
#' 1. Trains the LSTM model with early stopping
#' 2. Evaluates on validation and test sets
#' 3. Computes metrics at multiple forecast horizons
#' 4. Runs diagnostic checks for regression-to-mean
#' 5. Computes binary classification metrics
...
train_lstm_comprehensive <- function(lstm_data,
 ...
 if (is.null(lstm_data)) {
   cat(sprintf("ERROR: No data provided for %s\n", dataset_name))
   return(NULL)
 }
 ...
 cat(sprintf("  Features: %d (including %d autoregressive)\n", 
             lstm_data$n_features, length(lstm_data$ar_features)))
 cat(sprintf("  Training samples: %d\n", lstm_data$n_train))
 cat(sprintf("  Threshold for classification: %.2f\n", threshold))
 ...
 # =========================================================================
 # DIAGNOSTICS - Check for regression-to-mean
 # =========================================================================
 
 cat("\n")
 diagnostics <- diagnose_lstm_predictions(
 ...
 horizon_metrics <- data.frame(
   ...
   val_rmse = sapply(1:lstm_data$forecast_horizon, function(h) {
     sqrt(mean((val_pred[, h] - val_actual[, h])^2))
   }),
   val_mae = sapply(1:lstm_data$forecast_horizon, function(h) {
     mean(abs(val_pred[, h] - val_actual[, h]))
   }),
   test_rmse = sapply(1:lstm_data$forecast_horizon, function(h) {
     sqrt(mean((test_pred[, h] - test_actual[, h])^2))
   }),
   test_mae = sapply(1:lstm_data$forecast_horizon, function(h) {
     mean(abs(test_pred[, h] - test_actual[, h]))
   }),
   test_correlation = sapply(1:lstm_data$forecast_horizon, function(h) {
     cor(test_pred[, h], test_actual[, h])
   })
 )
 ...
 # 1-step metrics (most reliable)
 test_rmse_1step <- horizon_metrics$test_rmse[1]
 test_mae_1step <- horizon_metrics$test_mae[1]
 test_cor_1step <- horizon_metrics$test_correlation[1]
 
 # Intermediate horizons
 intermediate_days <- c(1, 7, 14, 21, 30, 40)
 intermediate_days <- intermediate_days[intermediate_days <= lstm_data$forecast_horizon]
 
 intermediate_metrics <- horizon_metrics |>
   filter(horizon_day %in% intermediate_days) |>
   select(horizon_day, test_rmse, test_mae, test_correlation)
 ...
 cat("\nClassification Results by Horizon:\n")
 ...
 cat(sprintf("  Test RMSE (all horizons): %.4f\n", test_rmse_all))
 cat(sprintf("  Test RMSE (1-step): %.4f\n", test_rmse_1step))
 cat(sprintf("  Test Correlation (1-step): %.3f\n", test_cor_1step))
 cat(sprintf("  Variance Ratio: %.3f [%s]\n", diagnostics$variance_ratio, 
             diagnostics$var_status))
 cat(sprintf("  Classification Accuracy (1-step): %.1f%%\n", 
             classification_df$accuracy[1] * 100))
 cat(sprintf("  F1 Score (1-step): %.3f\n", classification_df$f1[1]))
 cat(sprintf("  Diagnostic Status: %s\n", diagnostics$overall_status))
 cat(strrep("=", 60), "\n")
 ...
 return(list(
   ...
   predictions = list(
     ...
     val_predicted = val_pred,
     test_actual = test_actual,
     test_predicted = test_pred,
     dates_val = lstm_data$dates_val,
   ...
   scalers = list(
     target_mean = lstm_data$target_mean,
     target_sd = lstm_data$target_sd
   ),
   confusion_1step = table(
     Actual = ifelse(test_actual[, 1] > threshold, "YES", "NO"),
     Predicted = ifelse(test_pred[, 1] > threshold, "YES", "NO")
   ),
   ar_features = lstm_data$ar_features,
   lstm_data_summary = list(
     n_train = lstm_data$n_train,
     n_val = lstm_data$n_val,
     n_test = lstm_data$n_test,
...
#' Create comprehensive validation plots for LSTM results
#' Includes diagnostic plots to verify autoregressive approach is working
...
create_lstm_validation_plots <- function(lstm_result, output_dir = "./Output/LSTM") {
 ...
 diag <- lstm_result$diagnostics
 
 plots <- list()
 
 # =========================================================================
 # 1. Test Time Series - Actual vs Predicted
 # =========================================================================
 ...
 plots$test_timeseries <- ggplot(test_df, aes(x = date, y = value, color = type)) +
   ...
   labs(
     title = paste(dataset_name, "- Test Set Predictions (1-Step Ahead)"),
     subtitle = sprintf("RMSE: %.3f | Correlation: %.3f | Variance Ratio: %.3f",
                        diag$rmse, diag$correlation, diag$variance_ratio),
     x = "Date",
 ...
 ggsave(file.path(output_dir, paste0(gsub(" ", "_", dataset_name), "_test_timeseries.png")),
        plots$test_timeseries, width = 14, height = 6)
 ...
 plots$scatter <- ggplot(scatter_df, aes(x = actual, y = predicted)) +
   ...
   labs(
     title = paste(dataset_name, "- Predicted vs Actual"),
     subtitle = sprintf("R²: %.3f | Variance Ratio: %.3f [%s]",
                        diag$correlation^2, diag$variance_ratio, diag$var_status),
     x = "Actual (ft/yr)",
     y = "Predicted (ft/yr)"
   ) +
   theme_minimal() +
 ...
 # =========================================================================
 # 3. Variance Comparison - KEY DIAGNOSTIC PLOT
 # =========================================================================
 
 var_df <- data.frame(
   Type = c("Actual", "Predicted"),
   Variance = c(diag$actual_var, diag$pred_var)
 )
 
 plots$variance <- ggplot(var_df, aes(x = Type, y = Variance, fill = Type)) +
   geom_col(alpha = 0.8, width = 0.6) +
   geom_text(aes(label = sprintf("%.3f", Variance)), vjust = -0.5, size = 4) +
   scale_fill_manual(values = c("Actual" = "#2166AC", "Predicted" = "#D6604D")) +
   labs(
     title = paste(dataset_name, "- Variance Comparison"),
     subtitle = sprintf("Ratio: %.3f (Healthy: 0.8-1.2) | Status: %s",
                        diag$variance_ratio, diag$var_status),
     x = "",
     y = "Variance"
   ) +
   theme_minimal() +
 ...
 ggsave(file.path(output_dir, paste0(gsub(" ", "_", dataset_name), "_variance.png")),
        plots$variance, width = 6, height = 6)
 
 # =========================================================================
 # 4. Horizon Performance Degradation
 # =========================================================================
 
 horizon_df <- lstm_result$horizon_metrics
 
 plots$horizon_rmse <- ggplot(horizon_df, aes(x = horizon_day)) +
   geom_line(aes(y = test_rmse), color = "#D6604D", linewidth = 1.2) +
   geom_point(aes(y = test_rmse), color = "#D6604D", size = 2) +
   geom_line(aes(y = val_rmse), color = "#2166AC", linewidth = 1, linetype = "dashed") +
   geom_vline(xintercept = c(7, 14, 21, 30), linetype = "dotted", alpha = 0.5) +
   labs(
     title = paste(dataset_name, "- RMSE by Forecast Horizon"),
     subtitle = "Solid = Test, Dashed = Validation",
     x = "Forecast Horizon (days ahead)",
     y = "RMSE (ft/yr)"
   ) +
   theme_minimal()
 
 plots$horizon_cor <- ggplot(horizon_df, aes(x = horizon_day, y = test_correlation)) +
   ...
   labs(
     title = paste(dataset_name, "- Correlation by Forecast Horizon"),
     subtitle = "Green: Good (0.5), Orange: Minimum (0.3)",
     x = "Forecast Horizon (days ahead)",
     y = "Correlation"
   ) +
   theme_minimal()
 
 horizon_combined <- plots$horizon_rmse + plots$horizon_cor
 ggsave(file.path(output_dir, paste0(gsub(" ", "_", dataset_name), "_horizon.png")),
        horizon_combined, width = 14, height = 6)
 
 # =========================================================================
 # 5. Classification Performance by Horizon
 # =========================================================================
 ...
 if (!is.null(class_df) && nrow(class_df) > 0) {
   ...
   plots$classification <- ggplot(class_long, aes(x = horizon, y = value, color = metric)) +
     ...
     labs(
       title = paste(dataset_name, "- Classification Metrics by Horizon"),
 ...
 residuals_df <- data.frame(
   date = as.Date(preds$dates_test),
   residual = preds$test_actual[, 1] - preds$test_predicted[, 1],
   predicted = preds$test_predicted[, 1]
 )
 
 plots$residuals_time <- ggplot(residuals_df, aes(x = date, y = residual)) +
   geom_line(alpha = 0.6, color = "#4575B4") +
   geom_point(size = 1, alpha = 0.6) +
   geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
   geom_smooth(method = "loess", se = TRUE, color = "orange") +
   labs(
     title = paste(dataset_name, "- Residuals Over Time"),
     subtitle = "Look for patterns (should be random around zero)",
     x = "Date",
     y = "Residual (Actual - Predicted)"
   ) +
   theme_minimal()
 
 plots$residuals_fitted <- ggplot(residuals_df, aes(x = predicted, y = residual)) +
   geom_point(alpha = 0.6, color = "#4575B4") +
   geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
   geom_smooth(method = "loess", se = TRUE, color = "orange") +
   labs(
     title = paste(dataset_name, "- Residuals vs Fitted"),
     subtitle = "Look for heteroscedasticity",
     x = "Predicted Value",
     y = "Residual"
   ) +
   theme_minimal()
 
 residuals_combined <- plots$residuals_time + plots$residuals_fitted
 ggsave(file.path(output_dir, paste0(gsub(" ", "_", dataset_name), "_residuals.png")),
        residuals_combined, width = 14, height = 6)
 
 # =========================================================================
 # 7. Summary Dashboard
 # =========================================================================
 
 plots$summary <- (plots$test_timeseries) /
   (plots$scatter + plots$variance) /
   (plots$horizon_rmse + plots$horizon_cor) +
   plot_annotation(
     title = paste(dataset_name, "- LSTM Model Summary (Autoregressive)"),
     subtitle = sprintf("Diagnostic Status: %s | AR Features: %d",
                        diag$overall_status, length(lstm_result$ar_features))
   )
 
 ggsave(file.path(output_dir, paste0(gsub(" ", "_", dataset_name), "_summary.png")),
        plots$summary, width = 16, height = 18)
...
## Test autoregressive functions
```{r}
test_ar_configurations <- function(data, 
                                   gages_to_use, 
                                   threshold, 
                                   dataset_name,
                                   sequence_length = 30,
 ...
 cat(sprintf("\n%s\n", strrep("=", 70)))
 cat(sprintf("TESTING AUTOREGRESSIVE CONFIGURATIONS: %s\n", dataset_name))
 cat(sprintf("%s\n\n", strrep("=", 70)))
 
 # Define configurations to test
 configs <- list(
   "No_AR" = list(lags = c(), rolling = FALSE),
   "Lag_1_only" = list(lags = c(1), rolling = FALSE),
   "Lag_1_7" = list(lags = c(1, 7), rolling = FALSE),
   "Lag_1_7_14" = list(lags = c(1, 7, 14), rolling = FALSE),
   "Lag_1_7_14_30" = list(lags = c(1, 7, 14, 30), rolling = FALSE),
   "Full_AR" = list(lags = c(1, 7, 14, 30), rolling = TRUE)
 )
 ...
 for (config_name in names(configs)) {
   ...
   # Prepare data
   lstm_data <- tryCatch({
     prepare_lstm_data_autoregressive(
       data = data,
       gages_to_use = gages_to_use,
       sequence_length = sequence_length,
       forecast_horizon = forecast_horizon,
       dataset_name = paste(dataset_name, config_name),
       shoaling_lags = config$lags,
       include_rolling_stats = config$rolling
     )
   }, error = function(e) {
     cat(sprintf("  Data prep error: %s\n", e$message))
     NULL
   })
   ...
   if (is.null(lstm_data)) {
     results[[config_name]] <- list(
       config = config_name,
       status = "data_prep_failed",
       metrics = NULL
     )
     next
   }
   
   # Build and train model (quick training for comparison)
   model <- tryCatch({
     build_lstm_model(
       sequence_length = lstm_data$sequence_length,
       n_features = lstm_data$n_features,
       forecast_horizon = forecast_horizon,
       lstm_units = c(64, 32),  # Smaller for speed
       learning_rate = 0.001
     )
   }, error = function(e) {
     cat(sprintf("  Model build error: %s\n", e$message))
     NULL
   })
   ...
   cat(sprintf("  Results: RMSE=%.3f, Cor=%.3f, VarRatio=%.3f, Acc=%.1f%%\n",
               rmse, correlation, variance_ratio, accuracy * 100))
 ...
 comparison <- do.call(rbind, lapply(results, function(x) {
   if (x$status == "success") {
     data.frame(
       ...
       N_AR_Features = x$n_ar_features,
       RMSE = round(x$rmse, 4),
       MAE = round(x$mae, 4),
       Correlation = round(x$correlation, 3),
       Variance_Ratio = round(x$variance_ratio, 3),
       Accuracy = round(x$accuracy * 100, 1),
       Status = ifelse(x$variance_ratio >= 0.5 & x$correlation >= 0.3, 
                       "HEALTHY", "PROBLEMATIC")
     )
   } else {
     data.frame(
 ...
 cat("\n")
 cat(strrep("=", 70), "\n")
 cat("AUTOREGRESSIVE CONFIGURATION COMPARISON\n")
 cat(strrep("=", 70), "\n")
 print(comparison |> arrange(desc(Variance_Ratio)))
 
 # Find best configuration
 healthy_configs <- comparison |> filter(Status == "HEALTHY")
 if (nrow(healthy_configs) > 0) {
   best_config <- healthy_configs |> slice_min(RMSE, n = 1)
   cat(sprintf("\nBest healthy configuration: %s\n", best_config$Config))
   cat(sprintf("  RMSE: %.4f, Correlation: %.3f, Variance Ratio: %.3f\n",
               best_config$RMSE, best_config$Correlation, best_config$Variance_Ratio))
 } else {
   cat("\nWARNING: No healthy configurations found. Consider:\n")
   cat("  - Using more autoregressive lags\n")
   cat("  - Reducing forecast horizon\n")
   cat("  - Simpler model architecture\n")
 }
 
 return(list(
   results = results,
   comparison = comparison,
   best_config = if(nrow(healthy_configs) > 0) best_config$Config else "Full_AR",
   dataset_name = dataset_name
 ))
}
```
...
cat("\n")
cat(strrep("#", 70), "\n")
cat("# TRAINING RIVER-LEVEL LSTM MODELS WITH AUTOREGRESSIVE FEATURES\n")
cat(strrep("#", 70), "\n")

# ============================================================================
# STEP 1: Test autoregressive configurations
# ============================================================================

cat("\n### STEP 1: Testing Autoregressive Configurations ###\n")

IWW_ar_test <- test_ar_configurations(
 data = IWW_data,
...
Miss_ar_test <- test_ar_configurations(
 data = Miss_data,
...
# Save AR configuration comparison
ar_comparison <- bind_rows(
 IWW_ar_test$comparison |> mutate(River = "Illinois Waterway"),
 Miss_ar_test$comparison |> mutate(River = "Mississippi River")
)
write_csv(ar_comparison, "./Output/LSTM/AR_Configuration_Comparison.csv")

cat("\n### AR Configuration Comparison Saved ###\n")

# ============================================================================
# STEP 2: Train full models with best/default configuration
# ============================================================================

cat("\n### STEP 2: Training Full LSTM Models ###\n")

# Use Full_AR configuration (most robust based on ARA approach)
# Or use best_config from testing if available

# Illinois Waterway
cat("\n--- Training Illinois Waterway LSTM ---\n")
...
# Mississippi River
cat("\n--- Training Mississippi River LSTM ---\n")
...
## Generate River LSTM Validation Plots
...
if (!is.null(IWW_lstm_plots$summary)) {
 cat("\nIllinois Waterway Summary:\n")
 print(IWW_lstm_plots$summary)
}

if (!is.null(Miss_lstm_plots$summary)) {
 cat("\nMississippi River Summary:\n")
 print(Miss_lstm_plots$summary)
}
```

## Build Pool-Level Training LSTM Models

```{r train-lstm-pool-models}
#' Train LSTM with autoregressive features for a specific pool

train_pool_lstm <- function(data,
                           pool_name,
                           river_code,
                           pool_gage_mapping,
                           threshold,
 ...
 # Check minimum samples (need more for AR features due to lagging)
 min_required <- sequence_length + forecast_horizon + 50 + 30  # Extra for AR lags
 ...
 if (nrow(pool_data) < max(min_samples, min_required)) {
   return(list(
     ...
     n = nrow(pool_data),
     required = max(min_samples, min_required),
     metrics = NULL
   ))
 }
 ...
 # Prepare data with autoregressive features
 lstm_data <- tryCatch({
   prepare_lstm_data_autoregressive(
     data = pool_data,
     ...
     shoaling_lags = c(1, 7, 14),  # Fewer lags for smaller datasets
     include_rolling_stats = TRUE
   )
 }, error = function(e) {
   cat(sprintf("  Data prep error: %s\n", e$message))
 ...
 lstm_result <- tryCatch({
   train_lstm_comprehensive(
     ...
     batch_size = min(32, floor(lstm_data$n_train / 4)),
     patience = patience,
     lstm_units = c(64, 32),  # Smaller for pool-level
     learning_rate = 0.001
   )
 }, error = function(e) {
   cat(sprintf("  Training error: %s\n", e$message))
...
cat(sprintf("\nPools with sufficient data for LSTM: %d of %d\n", 
           nrow(lstm_viable_pools), nrow(viable_pools)))

if (nrow(lstm_viable_pools) > 0) {
 print(lstm_viable_pools |> select(pool, river, n_samples))
}

# Train models for each viable pool
...
for (i in 1:nrow(lstm_viable_pools)) {
 ...
 if (!is.null(result$status) && result$status == "success") {
   create_lstm_validation_plots(
     result, 
     output_dir = paste0("./Output/LSTM/Pool_", pool_name)
   )
 }
 
 # Clean up between pools
 keras3::clear_session()
 gc()
}
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("POOL-LEVEL LSTM RESULTS\n")
cat(strrep("=", 70), "\n")

if (!is.null(pool_lstm_metrics) && nrow(pool_lstm_metrics) > 0) {
 print(pool_lstm_metrics |>
         select(Pool, River, N_Samples, Test_RMSE_1Step, 
                Variance_Ratio, Accuracy_1Step, F1_1Step, Diagnostic_Status) |>
         arrange(Test_RMSE_1Step))
 
 write_csv(pool_lstm_metrics, "./Output/LSTM/Pool_Level_Metrics.csv")
} else {
 cat("No pool-level LSTM models trained successfully\n")
}
```
...
cat("\n")
cat(strrep("#", 80), "\n")
cat("#                    LSTM RESULTS SUMMARY (AUTOREGRESSIVE)                    #\n")
cat(strrep("#", 80), "\n")

# ============================================================================
# AR Configuration Comparison
# ============================================================================

cat("\n=== AUTOREGRESSIVE CONFIGURATION COMPARISON ===\n")
if (exists("ar_comparison") && !is.null(ar_comparison)) {
 print(ar_comparison |> 
         filter(Status == "HEALTHY" | !is.na(RMSE)) |>
         select(River, Config, RMSE, Correlation, Variance_Ratio, Accuracy, Status) |>
         arrange(River, desc(Variance_Ratio)))
}
...
river_lstm_metrics <- bind_rows(
 if (!is.null(IWW_lstm)) IWW_lstm$summary_metrics else NULL,
 if (!is.null(Miss_lstm)) Miss_lstm$summary_metrics else NULL,
 if (!is.null(Combined_lstm)) Combined_lstm$summary_metrics else NULL
)

if (!is.null(river_lstm_metrics) && nrow(river_lstm_metrics) > 0) {
 
 cat("\nRegression Metrics:\n")
 print(river_lstm_metrics |>
         select(Dataset, Test_RMSE_1Step, Test_MAE_1Step, 
                Test_Correlation_1Step, Variance_Ratio) |>
         mutate(across(where(is.numeric), ~round(., 3))))
 
 cat("\nClassification Metrics:\n")
 print(river_lstm_metrics |>
         select(Dataset, Accuracy_1Step, Precision_1Step, 
                Recall_1Step, F1_1Step) |>
         mutate(across(where(is.numeric), ~round(., 3))))
 
 cat("\nDiagnostic Status:\n")
 print(river_lstm_metrics |>
         select(Dataset, Variance_Ratio, Diagnostic_Status))
...
if (!is.null(pool_lstm_metrics) && nrow(pool_lstm_metrics) > 0) {
 ...
 pool_summary <- pool_lstm_metrics |>
   ...
   summarise(
     N_Pools = n(),
     N_Healthy = sum(Diagnostic_Status == "HEALTHY"),
     Avg_RMSE = round(mean(Test_RMSE_1Step, na.rm = TRUE), 3),
     Avg_Correlation = round(mean(Test_Correlation_1Step, na.rm = TRUE), 3),
     Avg_Variance_Ratio = round(mean(Variance_Ratio, na.rm = TRUE), 3),
...
comparison_df <- data.frame(
 River = c("Illinois Waterway", "Mississippi River", "Combined"),
 xGBoost_RMSE = c(
   IWW_xgb$metrics$Test_RMSE,
   Miss_xgb$metrics$Test_RMSE,
   Combined_xgb$metrics$Test_RMSE
 ),
 LSTM_RMSE = c(
   ...
   ifelse(!is.null(Combined_lstm), Combined_lstm$summary_metrics$Test_RMSE_1Step, NA)
 ),
 xGBoost_Accuracy = c(
   IWW_xgb$metrics$Accuracy * 100,
   Miss_xgb$metrics$Accuracy * 100,
   Combined_xgb$metrics$Accuracy * 100
 ),
 LSTM_Accuracy = c(
   ifelse(!is.null(IWW_lstm), IWW_lstm$summary_metrics$Accuracy_1Step * 100, NA),
   ifelse(!is.null(Miss_lstm), Miss_lstm$summary_metrics$Accuracy_1Step * 100, NA),
   ifelse(!is.null(Combined_lstm), Combined_lstm$summary_metrics$Accuracy_1Step * 100, NA)
 ),
 LSTM_Variance_Ratio = c(
   ifelse(!is.null(IWW_lstm), IWW_lstm$summary_metrics$Variance_Ratio, NA),
   ifelse(!is.null(Miss_lstm), Miss_lstm$summary_metrics$Variance_Ratio, NA),
   ifelse(!is.null(Combined_lstm), Combined_lstm$summary_metrics$Variance_Ratio, NA)
 ),
 LSTM_Status = c(
   ifelse(!is.null(IWW_lstm), IWW_lstm$summary_metrics$Diagnostic_Status, NA),
   ifelse(!is.null(Miss_lstm), Miss_lstm$summary_metrics$Diagnostic_Status, NA),
   ifelse(!is.null(Combined_lstm), Combined_lstm$summary_metrics$Diagnostic_Status, NA)
 )
) |>
 mutate(
   RMSE_Winner = case_when(
     is.na(LSTM_RMSE) ~ "xGBoost",
     xGBoost_RMSE < LSTM_RMSE ~ "xGBoost",
     TRUE ~ "LSTM"
   ),
   Accuracy_Winner = case_when(
     is.na(LSTM_Accuracy) ~ "xGBoost",
     xGBoost_Accuracy > LSTM_Accuracy ~ "xGBoost",
     TRUE ~ "LSTM"
   )
 )

print(comparison_df |>
       select(River, xGBoost_RMSE, LSTM_RMSE, RMSE_Winner,
              xGBoost_Accuracy, LSTM_Accuracy, Accuracy_Winner, LSTM_Status) |>
       mutate(across(where(is.numeric), ~round(., 3))))

write_csv(comparison_df, "./Output/Comparisons/LSTM_vs_xGBoost.csv")
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("KEY FINDINGS - AUTOREGRESSIVE LSTM\n")
cat(strrep("=", 70), "\n")

for (i in 1:nrow(comparison_df)) {
 cat(sprintf("\n%s:\n", comparison_df$River[i]))
 cat(sprintf("  xGBoost RMSE: %.3f | LSTM RMSE: %.3f → Winner: %s\n",
             comparison_df$xGBoost_RMSE[i],
             ifelse(is.na(comparison_df$LSTM_RMSE[i]), NA, comparison_df$LSTM_RMSE[i]),
             comparison_df$RMSE_Winner[i]))
 cat(sprintf("  xGBoost Acc: %.1f%% | LSTM Acc: %.1f%% → Winner: %s\n",
             comparison_df$xGBoost_Accuracy[i],
             ifelse(is.na(comparison_df$LSTM_Accuracy[i]), NA, comparison_df$LSTM_Accuracy[i]),
             comparison_df$Accuracy_Winner[i]))
 cat(sprintf("  LSTM Variance Ratio: %.3f | Status: %s\n",
             ifelse(is.na(comparison_df$LSTM_Variance_Ratio[i]), NA, 
                    comparison_df$LSTM_Variance_Ratio[i]),
             ifelse(is.na(comparison_df$LSTM_Status[i]), "N/A", comparison_df$LSTM_Status[i])))
}
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("AUTOREGRESSIVE APPROACH ASSESSMENT\n")
cat(strrep("=", 70), "\n")

# Check if AR approach fixed the regression-to-mean issue
healthy_count <- sum(comparison_df$LSTM_Status == "HEALTHY", na.rm = TRUE)
total_count <- sum(!is.na(comparison_df$LSTM_Status))

cat(sprintf("\nHealthy models (variance ratio >= 0.5, correlation >= 0.3): %d of %d\n",
           healthy_count, total_count))

if (healthy_count == total_count && total_count > 0) {
 cat("\n✓ SUCCESS: Autoregressive approach has resolved regression-to-mean behavior\n")
 cat("  All models now show meaningful predictive variance.\n")
} else if (healthy_count > 0) {
 cat("\n⚠ PARTIAL SUCCESS: Some models improved, others still problematic\n")
 cat("  Consider additional tuning for problematic models.\n")
} else {
 cat("\n✗ ISSUE: Models still showing regression-to-mean behavior\n")
 cat("  Consider: simpler architecture, 1-step prediction, more AR features\n")
}
...
if (nrow(horizon_data) > 0) {
 ...
 # RMSE by horizon
 p_horizon_rmse <- ggplot(horizon_data, aes(x = horizon_day, y = test_rmse, color = Dataset)) +
   geom_line(linewidth = 1.2) +
   geom_point(size = 2) +
   geom_vline(xintercept = c(7, 14, 21, 30, 40), linetype = "dashed", alpha = 0.3) +
   scale_color_viridis_d() +
   labs(
     title = "LSTM Forecast Error by Horizon (Autoregressive)",
     subtitle = "Vertical lines mark key decision horizons",
     x = "Forecast Horizon (days ahead)",
     y = "RMSE (ft/yr)"
   ) +
   theme_minimal() +
 ...
 p_horizon_cor <- ggplot(horizon_data, aes(x = horizon_day, y = test_correlation, color = Dataset)) +
   geom_line(linewidth = 1.2) +
   geom_point(size = 2) +
   geom_hline(yintercept = c(0.3, 0.5), linetype = "dashed", 
              color = c("orange", "green"), alpha = 0.7) +
   scale_color_viridis_d() +
   labs(
     title = "LSTM Correlation by Horizon",
     subtitle = "Green = Good (0.5), Orange = Minimum (0.3)",
     x = "Forecast Horizon (days ahead)",
     y = "Correlation"
   ) +
 ...
 horizon_combined <- p_horizon_rmse + p_horizon_cor
 print(horizon_combined)
 
 ggsave("./Output/LSTM/Horizon_Performance_All_AR.png", 
        horizon_combined, width = 14, height = 6)
}
```
...
cat("\n")
cat("╔════════════════════════════════════════════════════════════════════════╗\n")
cat("║                    POOL-LEVEL XGBOOST RESULTS                          ║\n")
cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")
...
cat("\n")
cat("╔════════════════════════════════════════════════════════════════════════╗\n")
cat("║                 POOL-LEVEL vs RIVER-LEVEL COMPARISON                   ║\n")
cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")
...
if (!is.null(pool_xgb_metrics) && nrow(pool_xgb_metrics) > 0) {
  ...
  p_rmse_r2 <- pool_xgb_metrics |>
    ...
    labs(
      title = "Pool Model Performance: RMSE vs R²",
      x = "Test RMSE (ft/yr)",
      y = "R²",
      color = "River",
      size = "N Samples"
...
cat("\n")
cat("╔════════════════════════════════════════════════════════════════════════╗\n")
cat("║                 COMPREHENSIVE POOL RESULTS TABLE                       ║\n")
cat("╚════════════════════════════════════════════════════════════════════════╝\n\n")
...
if (!is.null(pool_xgb_metrics) && nrow(pool_xgb_metrics) > 1) {
  ...
  cat(sprintf("  Interpretation: %s\n", 
              ifelse(cor_test$estimate < 0, 
                     "More samples → Lower RMSE (better)", 
                     "More samples → Higher RMSE (unexpected)")))
}
...
if (best_ml$Model == "LSTM") {
  cat("  → Use for: Long-range planning (30-45 days ahead)\n")
} else {
