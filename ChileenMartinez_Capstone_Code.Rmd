...
IWW_Viz <- gage_CSAT_joined |>
  ...
  select(SurveyDateBefore, Season, Day, Month, WeekOfYear,
         river, pool) |>
  mutate(
    season = factor(Season, levels = c("Winter", "Spring", "Summer", "Fall")),
    river_system = factor(river),
...
```{r}
build_forecast_lstm_model <- function(sequence_length, n_features, forecast_horizon) {
  model <- keras_model_sequential() |>
    # First LSTM layer - unidirectional for proper forecasting
    layer_lstm(
      units = 128,
      return_sequences = TRUE,
      input_shape = c(sequence_length, n_features),
...
## Prepare the data
```{r}
prepare_forecast_data <- function(data, gages_to_use,
                                 sequence_length = 30,
                                 forecast_horizon = 28,
                                 dataset_name = "Dataset") {
  
  cat(sprintf("\n--- Preparing Enhanced Forecast Data: %s ---\n", dataset_name))
  
  # Sort and prepare data with enhanced temporal features
  feature_data <- data |>
    select(
      SurveyDateBefore, AnnualShoalingRate_ftperyr,
      DaysBetween, WeekOfYear, any_of(gages_to_use)
    ...
    mutate(
      ...
      # Calendar features
      month = month(SurveyDateBefore),
      day_of_month = day(SurveyDateBefore),
      ...
      target_lag1 = lag(AnnualShoalingRate_ftperyr, 1),
      target_lag2 = lag(AnnualShoalingRate_ftperyr, 2),
      target_lag3 = lag(AnnualShoalingRate_ftperyr, 3),
      
      # Rolling statistics of the target
      target_rolling_mean = rollapply(AnnualShoalingRate_ftperyr, 3, mean, fill = NA, align = "right"),
      target_rolling_std = rollapply(AnnualShoalingRate_ftperyr, 3, sd, fill = NA, align = "right")
    ) |>
    # Remove rows with NAs from the lag features
    filter(row_number() > 3)
  ...
  gage_cols <- intersect(gages_to_use, names(feature_data))
  feature_cols <- c(
    "DaysBetween", "WeekOfYear", "days_since_last", 
    "month_sin", "month_cos", "week_sin", "week_cos",
    "target_lag1", "target_lag2", "target_lag3",
    "target_rolling_mean", "target_rolling_std",
    gage_cols
  )
  ...
  for (i in 1:n_samples) {
    X[i, , ] <- scaled_features[i:(i + sequence_length - 1), ]
    y[i, ] <- scaled_target[(i + sequence_length):(i + sequence_length + forecast_horizon - 1)]
    sample_dates[i] <- as.character(dates[i + sequence_length - 1])
    forecast_dates[[i]] <- dates[(i + sequence_length):(i + sequence_length + forecast_horizon - 1)]
  }
  ...
  lstm_data <- list(
    ...
    dates_val = sample_dates[(train_size + 1):(train_size + val_size)],
    dates_test = sample_dates[(train_size + val_size + 1):n_samples],
    forecast_dates_test = forecast_dates[(train_size + val_size + 1):n_samples],
    feature_means = feature_means,
    feature_sds = feature_sds,
    target_mean = target_mean,
    target_sd = target_sd,
...
### Train the model
```{r}
train_forecast_lstm_model <- function(lstm_data, dataset_name,
                                     epochs = 150, batch_size = 32, patience = 25, threshold) {
  ...
  cat(sprintf("\n=== Training Forecast LSTM: %s ===\n", dataset_name))
  
  # Build model
  model <- build_forecast_lstm_model(
  ...
  callbacks <- list(
    ...
    callback_model_checkpoint(
      filepath = paste0("lstm_", gsub(" ", "_", dataset_name), "_checkpoint"),
      save_best_only = TRUE,
      monitor = "val_loss"
    )
  )
  ...
  # Weekly metrics (for 28-day forecasts)
  if (lstm_data$forecast_horizon >= 28) {
    weekly_metrics <- data.frame(
      Week = 1:4,
      RMSE = sapply(1:4, function(w) {
        start_idx <- (w-1)*7 + 1
        end_idx <- min(w*7, lstm_data$forecast_horizon)
        sqrt(mean((test_pred[, start_idx:end_idx] - test_actual[, start_idx:end_idx])^2))
      }),
      MAE = sapply(1:4, function(w) {
        start_idx <- (w-1)*7 + 1
        end_idx <- min(w*7, lstm_data$forecast_horizon)
        mean(abs(test_pred[, start_idx:end_idx] - test_actual[, start_idx:end_idx]))
      })
    )
  ...
  class_metrics <- calculate_classification_metrics(test_actual[, 1], test_pred[, 1], threshold)
  ...
  plot_sample_forecasts <- function(n_samples = 3) {
    par(mfrow = c(n_samples, 1), mar = c(4, 4, 2, 1))
    
    # Select random samples
    sample_indices <- sample(1:nrow(test_pred), n_samples)
    ...
    for (idx in sample_indices) {
      ...
      # Create x-axis (days ahead)
      days_ahead <- 1:length(actual)
      
      # Plot
      plot(days_ahead, actual, type = "l", col = "blue", 
           main = paste("Forecast from", lstm_data$dates_test[idx]),
           xlab = "Days Ahead", ylab = "Shoaling Rate (ft/yr)")
      lines(days_ahead, pred, col = "red")
  ...
  # Plot sample forecasts
  plot_sample_forecasts(3)
  
  return(list(
    model = model,
    history = list(phase1 = history1, phase2 = history2),
    dataset_name = dataset_name,
    ...
    weekly_metrics = if (exists("weekly_metrics")) weekly_metrics else NULL,
    predictions = list(
      actual = test_actual,
      predicted = test_pred,
      dates_test = lstm_data$dates_test,
      forecast_dates = lstm_data$forecast_dates_test
    ),
    confusion = class_metrics$confusion,
    scalers = list(
      feature_means = lstm_data$feature_means,
      feature_sds = lstm_data$feature_sds,
      target_mean = lstm_data$target_mean,
      target_sd = lstm_data$target_sd
    ),
    plot_forecasts = plot_sample_forecasts
  ))
}

# Function to generate forecasts from new data
generate_lstm_forecast <- function(model_result, new_data, forecast_days = 28) {
  # Extract model and scaling parameters
  model <- model_result$model
  feature_means <- model_result$scalers$feature_means
  feature_sds <- model_result$scalers$feature_sds
  target_mean <- model_result$scalers$target_mean
  target_sd <- model_result$scalers$target_sd
  sequence_length <- model_result$model$input_shape[[2]]
  feature_names <- names(feature_means)
  ...
  # Scale features
  scaled_features <- scale(new_features, center = feature_means, scale = feature_sds)
  
  # Create input sequence
  X_new <- array(dim = c(1, sequence_length, ncol(scaled_features)))
  X_new[1, , ] <- scaled_features[(nrow(scaled_features) - sequence_length + 1):nrow(scaled_features), ]
  
  # Generate prediction
  pred_scaled <- predict(model, X_new)
  pred <- pred_scaled * target_sd + target_mean
  
  # Create forecast dates
  last_date <- new_data$SurveyDateBefore[nrow(new_data)]
  forecast_dates <- seq(last_date + 1, by = "day", length.out = length(pred))
  
  # Create weekly summaries
  weeks <- ceiling(seq_along(forecast_dates) / 7)
  weekly_pred <- sapply(unique(weeks), function(w) mean(pred[1, weeks == w]))
  weekly_dates <- sapply(unique(weeks), function(w) {
    format(median(forecast_dates[weeks == w]), "%Y-%m-%d")
  })
  ...
  return(list(
    daily_forecast = data.frame(
      Date = forecast_dates,
      ShoalingRate = as.numeric(pred[1, ])
    ),
    weekly_forecast = data.frame(
      Week = unique(weeks),
      WeekEnding = weekly_dates,
      ShoalingRate = weekly_pred
    ),
    model_name = model_result$dataset_name
  ))
}

# Test river-level implementation
test_river_level_lstm <- function() {
  cat("Testing River-Level LSTM Implementation\n")
  
  # Prepare data for Illinois Waterway
  IWW_forecast_data <- prepare_forecast_data(
    IWW_data, IWW_gages,
    sequence_length = 30, forecast_horizon = 28,
    dataset_name = "Illinois Waterway"
  )
  
  # Train model
  IWW_lstm_model <- train_forecast_lstm_model(
    IWW_forecast_data, "Illinois Waterway", 
    threshold = IWW_thresh
  )
  
  # Generate a forecast using the most recent data
  recent_data <- tail(IWW_data, 30 + 10)  # Get enough data for sequence + buffer
  IWW_forecast <- generate_lstm_forecast(
    IWW_lstm_model, recent_data, forecast_days = 28
  )
  
  # Plot the forecast
  plot(
    IWW_forecast$daily_forecast$Date, 
    IWW_forecast$daily_forecast$ShoalingRate,
    type = "l", col = "red", 
    main = "28-Day Illinois Waterway Shoaling Forecast",
    xlab = "Date", ylab = "Shoaling Rate (ft/yr)"
  )
  abline(h = IWW_thresh, lty = 2, col = "blue")
  legend("topright", legend = c("Forecast", "Dredging Threshold"), 
         col = c("red", "blue"), lty = c(1, 2))
  
  # Return the model and forecast
  return(list(model = IWW_lstm_model, forecast = IWW_forecast))
}

# Test pool-level implementation
test_pool_level_lstm <- function(pool_name = "LA", river_code = "IL") {
  cat(sprintf("Testing Pool-Level LSTM Implementation for %s (%s)\n", 
              pool_name, river_code))
  
  # Get the relevant gages for this pool
  key <- paste(river_code, pool_name, sep = "_")
  if (!key %in% names(pool_gage_mapping)) {
    stop(paste("No mapping found for pool:", pool_name))
  }
  ...
  if (nrow(pool_data) < 100) {
    stop("Insufficient data for this pool")
  }
  
  # Prepare data
  pool_forecast_data <- prepare_forecast_data(
    pool_data, selected_gages,
    sequence_length = 20, forecast_horizon = 28,
    dataset_name = paste("Pool", pool_name)
  )
  
  # Train model
  pool_lstm_model <- train_forecast_lstm_model(
    pool_forecast_data, paste("Pool", pool_name),
    threshold = pool_thresholds |> filter(pool == pool_name) |> pull(mean_rate)
  )
  
  # Generate a forecast using the most recent data
  recent_data <- tail(pool_data, 20 + 10)  # Get enough data for sequence + buffer
  pool_forecast <- generate_lstm_forecast(
    pool_lstm_model, recent_data, forecast_days = 28
  )
  
  # Plot the forecast
  plot(
    pool_forecast$daily_forecast$Date, 
    pool_forecast$daily_forecast$ShoalingRate,
    type = "l", col = "red", 
    main = paste("28-Day", pool_name, "Pool Shoaling Forecast"),
    xlab = "Date", ylab = "Shoaling Rate (ft/yr)"
  )
  abline(h = pool_thresholds |> filter(pool == pool_name) |> pull(mean_rate), 
         lty = 2, col = "blue")
  legend("topright", legend = c("Forecast", "Dredging Threshold"), 
         col = c("red", "blue"), lty = c(1, 2))
  
  # Return the model and forecast
  return(list(model = pool_lstm_model, forecast = pool_forecast))
}

# Run the tests
river_test <- test_river_level_lstm()
pool_test <- test_pool_level_lstm("MARSEILLES", "IL")
```
