...
dir.create("./Output/Pool_Models", showWarnings = FALSE)
dir.create("./Output/LSTM_Models", showWarnings = FALSE)
...
```{r}
### Lets look at missing values in the non-date related variables
...
ggplot(gage_CSAT_joined,aes(x = SurveyDateAfter, y = AnnualShoalingRate_ftperyr, color = river))+
         geom_line()+ 
  scale_color_manual(values = c("IL" = "orange", "UM" = "dodgerblue"),
    labels = c("IL" = "Illinois Waterway", "UM" = "Mississippi River"),
    name = "River")+ 
  labs(x = "Survey Date After", y = "Annual Shoaling Rate (ft/yr)", title = "Annual Shoaling Rates By Pool")+
  facet_wrap(~pool)
...
pool_samples <- CSAT_data_cleaned |>
    ...
    summarise(
      n_samples = n(),
      min_date = min(SurveyDateAfter),
      max_date = max(SurveyDateAfter),
    ...
    mutate(
      river_code = ifelse(river == "IL", "IWW", "Miss"),
...
pool_gages <- gage_metadata |>
  ...
  pool_info <- pool_samples |>
    left_join(
      pool_gages,
      by = c("pool" = "Pool", "river_code" = "River")
    )
...
select_pool_gages <- function(target_pool, target_river, metadata, 
 ...
 # Get gages in the target pool
  ...
  if(nrow(pool_gages) == 0) {
    return(NULL)
  }
  
  pool_max_rm <- max(pool_gages$RiverMile, na.rm = TRUE)
  pool_min_rm <- min(pool_gages$RiverMile, na.rm = TRUE)
  
  # Start with gages directly in the pool
  selected <- pool_gages$Gage_ID
  ...
  # Add tributary gages (if enabled)
  if(include_tributaries) {
    # Tributaries in this pool
    pool_tribs <- metadata |>
...
create_pool_gage_mapping <- function(pool_info, metadata, upstream_distance = 100) {
  ...
  for(i in 1:nrow(pool_info)) {
    ...
    river_code <- pool_info$river_code[i]
    
    key <- paste(river_code, pool, sep = "_")
    
    gages <- select_pool_gages(
      target_pool = pool,
      target_river = river_code,
      metadata = metadata,
    ...
    mapping[[key]] <- list(
      ...
      river_filter = ifelse(river_code == "IWW", "IL", "UM"),
...
### Create Temporal Splits function
```{r}
### One of the key portions of this modeling is to make temporal splits since
### this is a forecasting application. Lets use a 70/15/15 training, validation,
### testing split.\
...
Combined_baselines <- create_time_series_baselines(gage_CSAT_joined |>
                                                     mutate(pool = as.factor(pool)), Combined_splits, "Combined")
...
visualization_data <- gage_CSAT_joined |>
  slice(as.numeric(rownames(pca_data_complete))) |>  
...
IWW_top_loadings <- get_top_loadings(IWW_PCA) |>
  mutate(Analysis = "IWW Only")
...
train_xgboost_temporal <- function(data, splits, gages_to_use, dataset_name) {
  ...
  test_data <- data[splits$test_idx, ] |>
    select(AnnualShoalingRate_ftperyr, WeekOfYear,any_of(gages_to_use),pool, SurveyDateAfter) |>
    drop_na()
...
train_pool_xgboost <- function(data, pool_name, river_code, 
                                pool_gage_mapping, gage_metadata,
                                min_samples = 50) {
  ...
  if(!key %in% names(pool_gage_mapping)) {
    return(list(pool = pool_name, river = river_code, 
                status = "no_mapping", metrics = NULL))
  }
  ...
  if(nrow(pool_data) < min_samples) {
    return(list(pool = pool_name, river = river_code,
                status = "insufficient_samples", n = nrow(pool_data),
                metrics = NULL))
  }
  ...
  if(length(available_gages) < 2) {
    return(list(pool = pool_name, river = river_code,
                status = "insufficient_gages", n_gages = length(available_gages),
                metrics = NULL))
  }
  ...
  features <- pool_data |>
    select(
      ...
      WeekOfYear,
      DaysBetween,
      all_of(available_gages)
    ) |>
    drop_na()
  
  if(nrow(features) < 30) {
    return(list(pool = pool_name, river = river_code,
                status = "insufficient_complete", n_complete = nrow(features),
                metrics = NULL))
  }
  ...
  if(nrow(test_data) < 5) {
    return(list(pool = pool_name, river = river_code,
                status = "insufficient_test", metrics = NULL))
  }
  ...
  if(is.null(model)) {
    return(list(pool = pool_name, river = river_code,
                status = "training_failed", metrics = NULL))
  }
  ...
  actual_bin <- ifelse(actual > 0, "YES", "NO")
  pred_bin <- ifelse(pred > 0, "YES", "NO")
  accuracy <- mean(actual_bin == pred_bin)
...
prepare_lstm_data_fixed <- function(data, gages_to_use, 
  ...
  feature_data <- feature_data |>
    mutate(
      days_since_last = as.numeric(SurveyDateAfter - lag(SurveyDateAfter)),
      days_since_last = ifelse(is.na(days_since_last), 0, days_since_last)
    )
  
  # Report on temporal spacing
  gap_stats <- summary(feature_data$days_since_last[-1])
  cat(sprintf("Temporal gap statistics (days): Min=%.0f, Median=%.0f, Mean=%.1f, Max=%.0f\n",
  ...
  # Create sequences
  # For multi-step forecasting with irregular data:
  # We use observation indices rather than time steps
  # But include the days_since_last feature to help model learn timing
  ...
  y <- matrix(0, nrow = n_samples, ncol = forecast_horizon)
  sample_dates <- character(n_samples)
  cumulative_gaps <- numeric(n_samples)
  
  for(i in 1:n_samples) {
    # Input sequence
    X[i, , ] <- scaled_features[i:(i + sequence_length - 1), ]
    
    # Output: next forecast_horizon observations (scaled)
    y[i, ] <- scaled_target[(i + sequence_length):(i + sequence_length + forecast_horizon - 1)]
    
    # Record the date of the last prediction point
    sample_dates[i] <- as.character(dates[i + sequence_length + forecast_horizon - 1])
    
    # Record total time span covered by this sequence
    cumulative_gaps[i] <- sum(feature_data$days_since_last[i:(i + sequence_length + forecast_horizon - 1)])
  }
  
  cat(sprintf("Sequences created: %d\n", n_samples))
  cat(sprintf("Input shape: [%d, %d, %d]\n", n_samples, sequence_length, n_features))
  cat(sprintf("Output shape: [%d, %d]\n", n_samples, forecast_horizon))
  cat(sprintf("Average sequence time span: %.1f days\n", mean(cumulative_gaps)))
...
# 4.2 Improved LSTM model architecture
build_lstm_model <- function(sequence_length, n_features, forecast_horizon,
  ...
  model <- keras_model_sequential() |>
    ...
    layer_dropout(rate = dropout_rate * 0.5) |>
...
# 4.3 Train LSTM with improved callbacks and monitoring
train_lstm_model <- function(lstm_data, dataset_name,
  ...
  # Binary classification (using 1-step predictions)
  actual_binary <- ifelse(test_actual[, 1] > 0, "YES", "NO")
  pred_binary <- ifelse(test_pred[, 1] > 0, "YES", "NO")
  accuracy <- mean(actual_binary == pred_binary)
  
  TP <- sum(actual_binary == "YES" & pred_binary == "YES")
  FP <- sum(actual_binary == "NO" & pred_binary == "YES")
  FN <- sum(actual_binary == "YES" & pred_binary == "NO")
  TN <- sum(actual_binary == "NO" & pred_binary == "NO")
  ...
  cat(sprintf("  Classification - Accuracy: %.1f%%, Precision: %.1f%%, Recall: %.1f%%\n",
  ...
  return(list(
    ...
    confusion = table(Actual = actual_binary, Predicted = pred_binary),
...
train_pool_lstm <- function(data, pool_name, river_code,
  ...
  # LSTM needs more data than xGBoost
  ...
  if(is.null(lstm_data)) {
    return(list(pool = pool_name, river = river_code,
                status = "data_prep_failed", metrics = NULL))
  }
